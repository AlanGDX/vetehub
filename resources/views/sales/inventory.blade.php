                    ...
                    @forelse($products as $product)
                        <tr class="cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/30" data-cart-item
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right hidden" data-edit-cell>
                                <div class="inline-flex items-center gap-3 leading-none" data-edit-actions>
                                    @if(Route::has('products.edit'))
                                        <a href="{{ route('products.edit', $product) }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-900" title="Editar" aria-label="Editar articulo">
                                    ...
                                    @if(Route::has('products.destroy'))
                                        <form action="{{ route('products.destroy', $product) }}" method="POST" onsubmit="return confirm('Seguro que deseas eliminar este articulo?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center text-red-600 hover:text-red-900" title="Eliminar" aria-label="Eliminar articulo">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
